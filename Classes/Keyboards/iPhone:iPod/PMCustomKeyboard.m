...
- (id)init {
	...
	int nHt = 216; // 320 width in theory
	UIDeviceOrientation orientation = [[UIDevice currentDevice] orientation];
	if(UIDeviceOrientationIsLandscape(orientation))
		nHt = 162; // 480 width in theory
	...
	// space button
	[self.spaceButton setBackgroundImage:[PMCustomKeyboard imageFromColor:[UIColor colorWithWhite:0.5 alpha:0.5]] forState:UIControlStateHighlighted];
...
-(void)setTextView:(id<UITextInput>)textView {
	...
	// we have to do this so that we can override the current keyboard
